...
<template>
    <Head title="Вход в систему | BioBook" />
        ...
        <div class="auth-card glass-panel w-full sm:max-w-md relative z-10 overflow-hidden">
            <div class="p-8 md:p-10">
                <div class="mb-10 text-center">
                    ...
                    <p class="text-indigo-200/60 text-sm">Войдите, чтобы управлять своими книгами</p>
                </div>

                <div v-if="status" class="mb-6 p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm text-center">
                    {{ status }}
                </div>
                ...
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="input-field">
                        <div class="flex items-center justify-between mb-2 ml-1">
                            <label class="block text-xs font-bold text-indigo-300 uppercase tracking-widest">Пароль</label>
                            <Link v-if="canResetPassword" :href="route('password.request')" class="text-[10px] uppercase font-black text-indigo-400 hover:text-indigo-300 transition-colors">Забыли?</Link>
                        </div>
                        <input
                    ...
                    <div class="flex items-center ml-1">
                        <label class="flex items-center cursor-pointer group">
                            <div class="relative flex items-center">
                                ...
                                <div class="w-5 h-5 border-2 border-indigo-500/30 rounded-md bg-indigo-500/5 group-hover:bg-indigo-500/10 transition-colors flex items-center justify-center">
                                    <div v-if="form.remember" class="w-3 h-3 bg-indigo-500 rounded-sm"></div>
                                </div>
                            </div>
                            <span class="ml-3 text-sm text-indigo-200/60 font-medium select-none">Запомнить меня</span>
                        </label>
                    </div>
                    ...
                    <button
                        ...
                        class="premium-btn w-full py-4 mt-2"
                    >
                ...
                <div class="mt-10 pt-6 border-t border-white/5 text-center">
                    <p class="text-sm text-indigo-200/50">
                        Нет аккаунта?
                        <Link :href="route('register')" class="text-indigo-400 font-bold hover:text-indigo-300 transition-colors ml-1">Создать аккаунт</Link>
                    </p>
                </div>
            </div>
        </div>
